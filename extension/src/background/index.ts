...
const API_URL = process.env.PLASMO_PUBLIC_API_URL || "http://192.168.178.29:3001"
...
type Request =
  ...
  | { name: "apollon-summarize"; dataId: string; base64: string; mimetype: string }
...
async function getSummary(dataId: string): Promise<SummaryResult | null> {
  const res = await fetch(`${API_URL}/api/summary/${encodeURIComponent(dataId)}`)
  ...
  return { originalText: json.originalText, summary: json.summary, cached: true }
}
